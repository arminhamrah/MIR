...
def merge_perf_pure(perf_csv):
    # Derive pure CSV path in same folder
    ...
    for m in measures:
        ...
        # Serialize feature rows as JSON lists
        x = perf_sec[['note','onset','offset','measure_start','measure_end','time_signature']].to_dict(orient='records')
        y = perf_sec[['note','onset','offset','measure_start','measure_end','time_signature']].to_dict(orient='records')
        records.append({'perf_csv': perf_csv, 'measure': m,
                        'x': json.dumps(x), 'y': json.dumps(y)})
